...
public class PropHierarchy
{
    ...
    public void AddEntry(Guid parentId, Guid instanceId, bool isRoom = false)
    {
        ...
        parent.AddEntry(parentId, instanceId, isRoom);
    }
    ...
    public PropHierarchy FindEntry(Guid instanceId)
    {
        ...
        foreach (PropHierarchy child in children)
        {
            ...
            if (currChild != null) return child;
        }
        return null;
    }
    ...
    public void PrintPropValues(int indent = 0)
    {
        ...
        Debug.Log($"{indentChars}instance: {InstanceId}, isRoot: {IsRoot}, isRoom: {IsRoom}, props: {propCount.Count()}, children: {children.Count()}");
    ...
    // Returns the sum for a prop with a given name for a the nearest room
    ...
    // Returns the sym for a prop with a given name for the current hieachy element
